...
class Saman extends PortAbstract implements PortInterface
{
    ...
    public function redirect()
    {
        $main_data = [
            'amount'        => $this->amount,
            'merchant'      => data_get($this->config,'gateway.saman.merchant'),
    ...
    {
        if (!$this->callbackUrl)
            $this->callbackUrl = data_get($this->config,'gateway.saman.callback-url');
    ...
    {
        $fields = array(
            "merchantID" => data_get($this->config,'gateway.saman.merchant'),
            "RefNum" => $this->refId,
            "password" => data_get($this->config,'gateway.saman.password'),
        );
